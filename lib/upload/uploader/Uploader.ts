...
interface DirectoryUploadOptions extends BaseOptions {
	destination: string
	directory: Directory
	client: WebDAVClient
}

interface BatchUploadOptions extends BaseOptions {
...
export class Uploader {
	...
	async batchUpload(
		...
		try {
			...
			const promise = this.uploadDirectory({
				...options,
				destination,
			...
			if (isCancel(error) || error instanceof UploadCancelledError || (error instanceof DOMException && error.name === 'AbortError')) {
	 ...
	 *
	 * @param options - the options for the directory upload
	 * @param options.destination Destination where to create the directory
	 * @param options.directory The directory to create
	 * @param options.client The cached WebDAV client
	 */
	private async createDirectory(options: DirectoryUploadOptions): Promise<Upload> {
		if (!options.directory.name) {
			throw new Error('Can not create empty directory')
		}

		const folderPath = normalize(`${options.destination}/${options.directory.name}`).replace(/\/$/, '')
		const rootPath = `${this.root.replace(/\/$/, '')}/${folderPath.replace(/^\//, '')}`

		// Add a new upload to the upload queue
		const currentUpload: Upload = new Upload(rootPath, false, 0, options.directory)
		if (options.signal) {
			options.signal.addEventListener('abort', currentUpload.cancel)
		}
		this._uploadQueue.push(currentUpload)

		try {
			// Add the request to the job queue -> wait for finish to resolve the promise
			return await this._jobQueue.add(async () => {
				currentUpload.status = UploadStatus.UPLOADING
				await options.client.createDirectory(folderPath, { signal: currentUpload.signal })
			...
			if (isCancel(error) || error instanceof UploadCancelledError || (error instanceof DOMException && error.name === 'AbortError')) {
				...
				logger.debug('Directory already exists, writing into it', { directory: options.directory.name })
	...
	private async uploadDirectory(options: BatchUploadOptions & DirectoryUploadOptions): Promise<Upload[]> {
		// we use an internal abort controller to also cancel uploads if an error happened.
		// So if a signal is provided we connect it to our controller.
		const internalAbortController = new AbortController()
		if (options.signal) {
			options.signal.addEventListener('abort', () => internalAbortController.abort())
		}

		const internalOptions = { ...options, signal: internalAbortController.signal }
		const folderPath = normalize(`${internalOptions.destination}/${internalOptions.directory.name}`).replace(/\/$/, '')

		// Let the user handle conflicts
		const selectedForUpload = await (internalOptions.callback?.(internalOptions.directory.children, folderPath) ?? internalOptions.directory.children)
		if (selectedForUpload === false) {
			logger.debug('Upload canceled by user', { directory: internalOptions.directory })
			throw new UploadCancelledError('Conflict resolution cancelled by user')
		} else if (selectedForUpload.length === 0 && internalOptions.directory.children.length > 0) {
			logger.debug('Skipping directory, as all files were skipped by user', { directory: internalOptions.directory })
			return []
		}

		logger.debug('Start directory upload', { directory: internalOptions.directory })
		const directories: Promise<Upload[]>[] = []
		const uploads: Promise<Upload>[] = []
		try {
			if (internalOptions.directory.name) {
				...
				uploads.push(this.createDirectory(internalOptions))
			...
			for (const node of selectedForUpload) {
				if (node instanceof Directory) {
					directories.push(this.uploadDirectory({ ...internalOptions, directory: node }))
				} else {
					uploads.push(this.upload(`${folderPath}/${node.name}`, node, { signal: internalOptions.signal }))
				}
			}

			const resolvedUploads = await Promise.all(uploads)
			const resolvedDirectoryUploads = await Promise.all(directories)
			return [resolvedUploads, ...resolvedDirectoryUploads].flat()
		} catch (e) {
	 ...
	 * @param options - Optional parameters
	 */
	async upload(destination: string, fileHandle: File | FileSystemFileEntry, options?: UploadOptions): Promise<Upload> {
		...
		if (isFileSystemFileEntry(fileHandle)) {
			fileHandle = await new Promise((resolve, reject) => (fileHandle as FileSystemFileEntry).file(resolve, reject))
		}
